...
router.post('/avatar', verifyToken, upload.single('avatar'), uploadToBlob, async (req, res) => {
    if (!req.file) {
        return res.status(400).json({ error: 'Nenhum arquivo enviado.' });
    }

    const { id } = req.user;
    const avatarUrl = req.file.path; // URL do Blob

    try {
        // Buscar avatar antigo para deletar
        const oldUser = await db.query('SELECT avatar_url FROM users WHERE id = $1', [id]);
        if (oldUser.rows[0]?.avatar_url) {
            await deleteFromBlob(oldUser.rows[0].avatar_url);
        }
